        ...
        <div className="grid lg:grid-cols-2 gap-8">
          {pillars.map((p, i) => (
            <motion.div
              ...
              style={{ background: `rgba(${p.color === "#00C0FF" ? "0,192,255" : "0,192,255"},0.05)`, borderColor: `rgba(${p.color === "#00C0FF" ? "0,192,255" : "0,192,255"},0.2)` }}
            >
              <div className="absolute top-0 left-0 right-0 h-px" style={{ background: `linear-gradient(90deg, transparent, ${p.color}, transparent)` }} />
              <div className="w-11 h-11 rounded-xl flex items-center justify-center mb-5" style={{ background: `rgba(${p.color === "#00C0FF" ? "0,192,255" : "0,192,255"},0.15)` }}>
                <p.icon className="w-5 h-5" style={{ color: p.color }} />
              </div>
              <p className="text-xs font-semibold tracking-widest uppercase mb-1" style={{ color: p.color }}>{p.label}</p>
              <h3 className="font-unbounded text-xl font-bold mb-1" style={{ color: "#E8F1FF" }}>{p.title}</h3>
              <p className="text-sm font-medium mb-4" style={{ color: p.color }}>{p.subtitle}</p>
              <p className="text-sm leading-relaxed mb-6" style={{ color: "#9CB2D8" }}>{p.body}</p>
